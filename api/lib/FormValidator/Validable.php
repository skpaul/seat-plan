    ...
    class Validable{
        ...
        #region Check for data type
            ...
            public function asInteger($allowNegative){
                ...
                if($this->required || ($this->required==false && isset($valueToValidate) && !empty($valueToValidate))){
            ...
            public function asBool(){
                $valueToValidate = $this->valueToValidate; //make it debug-friendly with xdebug.
                if($this->required || ($this->required==false && isset($valueToValidate) && strlen($valueToValidate) > 0)){
                    $true = ["yes","true","1"];
                    if(in_array(strtolower($valueToValidate),$true)){
                        $this->valueToValidate = true;
                    }
                    else{
                        $false = ["no","false","0"];
                        if(in_array(strtolower($valueToValidate),$false)){
            ...
            public function equalLength($length){
                ...
                if($this->required || ($this->required==false && isset($valueToValidate) && !empty($valueToValidate))){
                    if(!empty($valueToValidate)){
                        $_length = strlen($valueToValidate);
                        $label = $this->label;
                        if($_length != $length){
        ...
        public function validate(){
            $whatisValue = $this->valueToValidate;
            $temp = var_export($this->valueToValidate, true); //$this->valueToValidate;
            if(strlen($temp) == 0)
