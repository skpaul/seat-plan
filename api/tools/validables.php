        ...
        <div class="container"> 
            ...
            <div class="data-container">
                <?php
                    if(isset($_POST["submit"])){
                        $db->fetchObject();
                       $columns = $db->showColumns($table_name);
                       $allString = "";
                       $id = 1;
                       foreach ($columns as $column) {
                            ...
                            switch ($dataType) {
                                case 'tinyint':
                                    $asType = 'asBool()->maxLength('.$length.')->';
                                    ...
                                    break;
                                case 'int':
                                    $asType = 'asInteger(false)->maxLength('.$length.')->';
                                    if($column->Default == null){
                                        $default = 'default(NULL)->';
                                    }
                                    else{
                                    ...
                                    break;
                                case 'varchar':
                                    $asType = 'asString(true)->maxLength('.$length.')->';
                                    if($column->Default == null){
                                        $default = 'default(NULL)->';
                                    }
                                    else{
                                ...
                                case 'decimal':
                                    ...
                                    $asType = 'asFloat(true)->maxLength('.$floatLength.')->';
                                    if($column->Default == null){
                                        $default = 'default(NULL)->';
                                    }
                                    else{
                             ...
                             $validable = '$'.$table_name.'->'.$databaseColumn.'= $form->label("'.$label.'")->httpPost("'.$field.'")->'.$requiredOrOptional.''.$asType.$default .'validate();' ;
                            //$validable = '$educations->'.$databaseColumn.'= $form->label("'.$label.'")->httpPost("'.$field.'")->'.$requiredOrOptional.''.$asType.$default .'validate();' ;
